...
import styles from "./DelegacionProcesoActionMenu.module.css";
...
const MENU_ESTIMATED_HEIGHT = DELEGACION_PROCESO_SECTION_OPTIONS.length * 36 + 12;
...
export function DelegacionProcesoActionMenu({
  onSelect,
}: {
  onSelect: (section: DelegacionProcesoModalSection) => void;
}) {
  const menuId = useId();
  ...
  const menu =
    open && menuPosition ? (
      ...
      >
        {DELEGACION_PROCESO_SECTION_OPTIONS.map((option) => (
          <button
            key={option.id}
            type="button"
            role="menuitem"
