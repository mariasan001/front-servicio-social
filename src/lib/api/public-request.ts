...
import type { ApiResponse, ReportPageResponse } from "@/lib/api/types";
...
export async function publicApiGetPage<T>(
  path: string,
  revalidate = PUBLIC_API_REVALIDATE_SECONDS,
): Promise<PublicApiResult<T[]>> {
  try {
    ...
    const payload = (await response.json()) as ApiResponse<ReportPageResponse<T>>;

    if (!payload.data?.content) {
      return { ok: false, reason: "unavailable" };
    }

    return { ok: true, data: payload.data.content };
  } catch {
    return { ok: false, reason: "unavailable" };
  }
}
